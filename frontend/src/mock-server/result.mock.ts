import {CourseInfo} from '@/types';

export const MOCK_RESULT: CourseInfo[] = [
    {
        match: 0.89,
        months: 12,
        url: 'https://gb.ru/geek_university/developer/architecture/devops',
        title: 'DevOps-инженер с нуля до Middle',
        price: 5223,
        description:
            'Вы научитесь настраивать пайплайны CI/CD, использовать подход «Инфраструктура как код» для настройки серверов и развёртывания инфраструктуры в облаке. Узнаете, в чём преимущество микросервисной архитектуры по сравнению с традиционной. Сможете работать с Docker и Kubernetes.',
        technologies: [
            'Архитектура',
            'DevOps-инженер',
            'Ethernet',
            'Unix',
            'CI/CD',
            'DNS',
            'Docker',
            'Kubernetes (K8s)',
            'Python',
            'SQL',
            'NoSQL',
            'Zabbix',
            'ELK',
        ],
    },
    {
        match: 0.78,
        months: 4,
        url: 'https://gb.ru/geek_university/management/accountant',
        title: 'Профессия Бухгалтер',
        price: 4049,
        description:
            'Бухгалтер следит за финансами компании, работает с документами и отчетностью. Помогает бизнесу отслеживать изменения в законе и избегать убытков',
        technologies: [
            'Инструменты',
            '1С:Бухгалтерия 8, редакция 3.0',
            '1С:зарплата и управление персоналом 8',
            'Excel',
            'Google Таблицы',
            'Бухгалтерский баланс',
            'Навыки',
            'Работа с первичной бухгалтерий',
            'Ведение бухучета и финансовых операций',
            'Формирование бухгалтерской и налоговой отчетности',
            'Подготовка иллюстрированной айдентики',
            'Формирование бухгалтерских проводок',
            'Взаимодействие с налоговой, фондами и контрагентами',
            'Проведение инвентаризации, прохождение проверок',
            'Учет основных и нематериальных активов',
            'Организация документооборота',
            'Знание законодательства',
        ],
    },
    {
        match: 0.35,
        months: 6,
        url: 'https://gb.ru/geek_university/marketing/seo',
        title: 'Курс «SEO-специалист»',
        price: 2550,
        description:
            'Вы изучите основы маркетинга: от иследования рынка и проработки идеи продукта до UX- исследований и презентации их результатов заказчику.  После — погрузитесь в изучение SEO-продвижения, получите профильные навыки и начнете карьеру.',
        technologies: [
            'Яндекс.Метрика',
            'Яндекс.Директ',
            'Vk',
            'Telegram',
            'Google Ads',
            'Google Search Console',
            'Google Analytics',
        ],
    },
];
